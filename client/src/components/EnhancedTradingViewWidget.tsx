...
> = (
  ...
  const [studyIdsMap, setStudyIdsMap] = useState<Record<string, string>>({});
      ...
      try {
        ...
        if (!selectedIndicators.includes(indicator)) {
          ...
          try {
            ...
            // Store the study ID for later removal
            setStudyIdsMap(prev => ({
              ...prev,
              [indicator]: studyId
            }));
                ...
                constructor: function() {
                  this.init = function() {
                    this.exec = function() {
                      ...
                      const id = chart.createStudy(indicator);
                      // Store the study ID for later removal
                      setStudyIdsMap(prev => ({
                        ...prev,
                        [indicator]: id
  ...
  const handleRemoveIndicator = (indicator: string) => {
    if (widgetRef.current) {
      try {
        const studyId = studyIdsMap[indicator];
        
        if (studyId) {
          // Remove the study using the stored ID
          widgetRef.current.chart().removeEntity(studyId);
          
          // Update the maps
          const newStudyIdsMap = { ...studyIdsMap };
          delete newStudyIdsMap[indicator];
          setStudyIdsMap(newStudyIdsMap);
        } else {
          // Fallback method if we don't have the ID
          const allStudies = widgetRef.current.chart().getAllStudies();
          const study = allStudies.find((s: any) => s.name === indicator);
          
          if (study) {
            widgetRef.current.chart().removeEntity(study.id);
          }
        }
